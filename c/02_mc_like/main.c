...
typedef void (*update_and_draw_func)(State *state);

int main() {
  void *shared_lib_handle = NULL;
  update_and_draw_func update_and_draw = NULL;
  ...
  while (!WindowShouldClose()) {
    ...
    if (attr.st_mtime > last_modified) {
      ...
      update_and_draw =
          (update_and_draw_func)dlsym(shared_lib_handle, "update_and_draw");
      if (!update_and_draw) die("Error loading symbol: %s\n", dlerror());
